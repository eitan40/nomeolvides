...
public class Nomeolvides.ImportarHechos : Dialog {
	...
	private void elegir_archivo () {
		var abrir_archivo = new FileChooserDialog ( _("Select a file"), this as Window,
		                                            FileChooserAction.OPEN, _("Cancel"),
		                                            Gtk.ResponseType.CANCEL, _("Open"), 
		                                            ResponseType.ACCEPT	);
		abrir_archivo.set_current_folder ( this.directorio );

		abrir_archivo.close.connect ( this.se_manda_a_cerrar );
	...
	private void se_manda_a_cerrar  () {
		print ("Cerrate loco!\n");
	}
